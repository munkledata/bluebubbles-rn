...
|   8 | Remote TanStack reads and HTTP retry backoff                       | `queryClient.clear()` synchronously aborts the 15 remote query signals; every endpoint now forwards that signal to `HttpClient`; retry sleep is abortable | Generation-keyed cache plus screen lease rejects a late injected/non-abortable result                                | `httpBranches.test.ts`, endpoint suites, `sessionScopedState.test.ts`, Account/Server Management/Server Health component suites | Fixed in this batch |
...
There are 16 production TanStack query records: 15 remote account reads (Account 1, Server Management
3, Server Health 10, Backup 1) and one local encrypted-DB chat search. There are no production `fetchQuery`,
`prefetchQuery`, `ensureQueryData`, or mutation-cache call sites. The 15 remote functions consume
