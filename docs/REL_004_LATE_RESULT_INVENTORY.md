...
`QueryClient` surfaces. Production TanStack query functions currently do not consume the supplied
`AbortSignal`; their late publication is nevertheless fenced. Physical cancellation completeness
belongs to `REL-005A`, not this result-isolation milestone.
...
| Uploads                                               | Public cancellation, upload-store settlement, snapshotted A credentials, and result guards prevent a detached A native tail from adopting or publishing into B. The exact native `uploadAsync` tail is not yet drained before B admission.                 | upload-control and attachment-upload transport suites                         | Late publication safe; drain completeness deferred to `REL-005A` |
...
runs on a future candidate containing this post-v56 work. The lingering native upload tail and
TanStack transport cancellation are recorded for `REL-005A`; they do not use B credentials or
republish into B, but teardown completeness still requires their own bounded-drain proof.
