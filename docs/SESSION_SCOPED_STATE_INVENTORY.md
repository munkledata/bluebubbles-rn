...
| Upload store                    | Chat GUID, filename, and byte progress                 | Store reset; upload registry cancellation happens first, then exact native tails drain before B       |
...
| Upload control                           | FIFO gate, registered native cancel handles, and cancelled native tails                                     | Handles register before the first await; Disconnect sweeps twice and bounded-drains each exact terminal promise                                                                                                                                     |
...
| Scheduled/edit/send queues               | DB/generation recovery slots, per-message mutation tails, and the process-wide logical-send FIFO            | Persistent claims are account scoped; UI mutations capture a lease; FIFO payloads are snapshotted and retained callbacks are bounded; invalidation cancels queued old-generation work while active work remains tracked to real settlement          |
...
owning test files. The `REL-005A` post-v56 additions passed 9 Node suites / 207 tests and 3 component
suites / 66 tests, TypeScript, targeted lint/format, the 1,301-entry DB-write report with no errors,
and exact 0/0/0 reconciliation. This proof is compositional; it does not replace the end-to-end
device journey below.
