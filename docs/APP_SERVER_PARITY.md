...
- **`new-server`** (tunnel-URL rotation) — the app subscribes through `ServerUrlEventSink` → `applyNewServerUrl`, but RT-01A containment now quarantines every cross-origin/downgrade event before persistence or credential use. Automatic adoption remains intentionally disabled until a foreground approval + password-reconfirmation flow exists; users must reconnect explicitly after a real tunnel rotation.
...
| 🟡 Contained; approval UX open (`RT-01A`)                                                                      | socket-emit: new-server                                                                                                                            | Wired via `ServerUrlEventSink` → `applyNewServerUrl`, but cross-origin automatic adoption is quarantined so an event cannot silently redirect the password. Foreground approval + password reconfirmation remains open; until then a real tunnel rotation needs manual reconnect.                                                                                                                                    |
...
- **Server → App: mostly wired now.** The app uses the core data/action APIs fully, and the previously-untapped realtime + diagnostics surface has largely been closed (see the _✅ Closed (2026-07-01)_ section above): `message-send-error` and the admin/diagnostics reads (`get-private-api-status`, `get-env`→`findmyNeedsKeys`, `get-findmy-keys-status`, `get-fcm-status`, `get-alerts`, `get-config`) are handled — the diagnostics reads are surfaced by the in-app **Server Health** screen. `new-server` is parsed but deliberately quarantined pending the RT-01A approval flow described above. The app-relevant message/chat event differences intentionally left unresolved are:
