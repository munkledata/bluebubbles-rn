...
> Still open (lower-priority parity): scheduled recurrence, server-update
> install, QR display. (The multi-image gallery has since shipped too — a two-column collapse in
> `src/ui/attachments/AttachmentGalleryGrid.tsx`.) Package names + line counts below are stale. The
> table below is the ORIGINAL 2026-06-21 comparison.
